...
tags:
  ...
  - NIFs
private: false
updated_at: '2026-01-29T15:13:58+09:00'
...
my_nif_dest="$atomvm_esp32_path/components/hello_atomvm_nif"

# 既存があっても置き換える（-s: symlink, -f: force, -n: treat dest as normal file）
ln -sfn "$my_nif_src" "$my_nif_dest"
```

ここ（ビルドの直前）で、Elixir 用のパーティションテーブルを使うために `sdkconfig.defaults` に次を追記します。

```diff_bash:$atomvm_esp32_path/sdkconfig.defaults
+ CONFIG_PARTITION_TABLE_CUSTOM_FILENAME="partitions-elixir.csv"
```

https://qiita.com/mnishiguchi/items/caa55d2402e2c7d1304b

ESP-IDF の環境を読み込み、AtomVM（ESP32 向け）をビルドして書き込みます。
...
同じ要領でPortも使えることがわかりました。

https://github.com/piyopiyoex/hello_atomvm_port
